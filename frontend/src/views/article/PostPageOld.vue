<template>
  ...
  >
    ...
    <div class="list-section">
      <div class="list-header immersive-only">
        <h2 class="list-title">最新文章</h2>
      </div>

      <!-- posts 模式下的 tab 切换 -->
      <div class="tab-header posts-only">
        <button
      ...
      <!-- immersive 模式底部按钮 -->
      <div class="list-footer immersive-only">
...
/* 沉浸/文章模式内容双向淡入淡出（替代 v-show，由 CSS opacity + max-height 驱动） */
.immersive-only {
  opacity: 1;
  max-height: 80px;
  overflow: hidden;
  transition: opacity 0.35s ease-in-out, max-height 0.35s ease-in-out, padding 0.35s ease-in-out;
}

.mode-posts .immersive-only {
  opacity: 0;
  max-height: 0;
